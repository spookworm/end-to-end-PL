function [input] = WaveletE(input)
Nfft = 512;                 fsamples = Nfft/2; 
df = 60e6 / fsamples;       dt = 1 / (df*Nfft);
...
figure; 
subplot(2,1,1); 
plot((0:fsamples-1)*df/1e6,abs(Wavelfrq(1:fsamples)),'b','LineWidth',1.5);  
  axis('tight');
  xlabel(' frequency  [MHz] \rightarrow ')
  ylabel(' |W(f)|  \rightarrow ');
  legend('frequency amplitude spectrum'); 
subplot(2,1,2); 
plot((0:fsamples-1)*dt*1e6,Waveltime(1:fsamples),'r','LineWidth',1.5);
  axis('tight');
  xlabel(' time  [{\mu}s] \rightarrow ') 
